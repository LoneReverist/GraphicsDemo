...
{
public:
	template<details::VertexConcept Vertex>
	Mesh(std::vector<Vertex> && verts, std::vector<unsigned int> && indices)
		: m_mesh_var(details::MeshImpl<Vertex>{ std::move(verts), std::move(indices) })
	{}
...
private:
	using mesh_variant_t = std::variant<details::MeshImpl<BasicVertex>, details::MeshImpl<TextureVertex>>;
